...
class AssignmentController extends Controller
{
    ...
    public function myCoursesAssignments(Request $request)
    {
        ...
        $query->join('webinar_partner_teacher', function ($join) use ($user) {
            $join->on('webinar_assignments.webinar_id', '=', 'webinar_partner_teacher.webinar_id')
                 ->where('webinar_partner_teacher.teacher_id', '=', $user->id)
                 ->Orwhere('webinar_assignments.creator_id', '=', $user->id)
        ->whereColumn('webinar_assignments.webinar_id', '=', 'webinar_partner_teacher.webinar_id');// Added condition;
        })
        ->select('webinar_partner_teacher.teacher_id', 'webinar_partner_teacher.webinar_id','webinar_assignments.*');
        $courseAssignmentsCount = deepClone($query)->count();
    ...
    public function students(Request $request, $id)
    {
        ...
        if (!empty($assignment)) {
            ...
            $courseAssignmentsCount = WebinarAssignment::where('webinar_id', $webinar->id)
                ->count();
