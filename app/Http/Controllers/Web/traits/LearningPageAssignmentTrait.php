...
trait LearningPageAssignmentTrait
{
    ...
    private function getAssignmentData($course, $requestData)
    {
       ...
       $assignment = WebinarAssignment::join('webinar_partner_teacher', function ($join) {
            $join->on('webinar_assignments.webinar_id', '=', 'webinar_partner_teacher.webinar_id');
        })
        ->where('webinar_assignments.id', $requestData['item'])
        ->where('webinar_assignments.webinar_id', $course->id)
        ->first(['webinar_assignments.*', 'webinar_partner_teacher.teacher_id']);
        ...
        if (!empty($assignment)) {
            ...
            if (!empty($webinar) and $webinar->checkUserHasBought($user)) {
                ...
                $studentId = (!empty($requestData['student'])) ? $requestData['student'] : null;
