...
function getCourseProgressForStudent($webinar, $userId)
{
    $progress = 0;
...
{
    $user = auth()->user();
    $categoriesUser = collect([]);
    if (isset($user)) {
        if ($user->role['is_admin'] == 0 && $user->role['id'] == 1) {
            $categories = Category::where('slug', 'sub-general')
                ->with([
            ...
            if (isset($user->category_id)) {
                $categoriesUser = Category::where('id', $user->category_id)
                    ->orderBy('order', 'asc')->get();
                if ($categoriesUser[0]->parent_id == null) {
                    $categoriesUser = $categoriesUser->merge($categories);
                } else {
                    $categoriesUser = $categories;
                }
            }
        } else {
            $categoriesUser = Category::whereNull('parent_id')
                ->with([
                    'subCategories' => function ($query) {
                        $query->orderBy('order', 'asc');
                    },
